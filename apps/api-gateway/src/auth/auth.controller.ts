import { Controller, Post, Body, Inject, OnModuleInit, InternalServerErrorException } from '@nestjs/common';
import { ClientGrpc } from '@nestjs/microservices';
import { lastValueFrom, Observable } from 'rxjs';

interface AuthGrpcService {
  login(data: { usename: string; password: string }): Observable<{ accessToken: string; refreshToken: string }>;
  registerUser(data: { usename: string; password: string }): Observable<{ userId: string; message: string }>;
}
